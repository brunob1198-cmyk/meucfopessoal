...
export default function Inteligencia() {
  ...
  const [periodType, setPeriodType] = useState(() => localStorage.getItem('IA_PERIOD') || '12m');
  const [customStart, setCustomStart] = useState(() => localStorage.getItem('IA_START') || '');
  const [customEnd, setCustomEnd] = useState(() => localStorage.getItem('IA_END') || '');

  useEffect(() => {
    localStorage.setItem('IA_PERIOD', periodType);
    localStorage.setItem('IA_START', customStart);
    localStorage.setItem('IA_END', customEnd);
